...
- **콜아웃(prompt)**: `prompt-info` 사용. 색은 머스터드 `#dda233`, ⓘ 아이콘 제거, 콜아웃 안 리스트는 자체 여백 0(콜아웃 패딩이 간격 담당), 리스트 마커는 사각 불릿(▪, `list-style: square`). 모두 `assets/css/jekyll-theme-chirpy.scss`의 전역 CSS라 새 글에 자동 적용됨 — 포스트마다 손댈 필요 없음
