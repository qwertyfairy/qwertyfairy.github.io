...
- **삭제 예정 부분 붉은 형광펜(2026-07 확정)**: 삭제 승인을 받을 블록은 같은 방식으로 `<div class="preview-removed" markdown="1"> … </div>`(붉은 배경 + 점선 테두리, 샌드박스 scss의 `.preview-removed`)로 감싸 보여주고, 승인 후 본 저장소에서 블록을 실제 삭제한다. 배포 후 샌드박스는 배포본 파일로 덮어 동기화해 래퍼를 남기지 않는다
